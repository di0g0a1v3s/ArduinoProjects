  ...
  if(command.length()>0)
  {
    ...
    else if(on)
    {
      if(command.equals(INCREASE_INTENSITY_COMMAND))
      {
        ...
        led_brightness += 2;
        intensity = (int)led_brightness/3;
        if(led_brightness > 30) led_brightness = 30;
...
{
  led_brightness = map(ldr, 0, 1024, 0, 27);
  intensity = map(ldr, 0, 1024, 0, 9);
}
...
void displayTempAndHum()
{
  ...
  text[11] = humF[0];
  text[12] = humF[1];
  printStringWithShift(text, text_speed); // (text, scrolling speed)
}
...
void setRGBLed()
{
  ...
  {
    analogWrite(RedLedPin, 0);
    analogWrite(GreenLedPin, led_brightness);
    analogWrite(BlueLedPin, led_brightness);
  }
